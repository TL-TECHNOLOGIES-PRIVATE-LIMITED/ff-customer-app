...
class LocalAwesomeNotification {
  ...
  createImageNotification(
    ...
    try {
      print(
          "[Notification] Creating image notification with data: ${data.data}");
      int currentCount =
      ...
      print(
          "[Notification] New notification count: ${notificationCount.value}");
  ...
  createNotification(
    ...
    try {
      print(
          "[Notification] Creating simple notification with data: ${data.data}");
      int currentCount =
          Constant.session.getIntData(SessionManager.notificationTotalCount);
      Constant.session
          .setIntData(SessionManager.notificationTotalCount, currentCount + 1);
  ...
  requestPermission({required BuildContext context}) async {
    try {
      ...
      if (notificationPermissionStatus.isPermanentlyDenied) {
        ...
        print("[Permission] Notification permission denied, requesting...");
      ...
      print("ERROR in requestPermission: ${e.toString()}");
    }
  }

  static Future<void> onBackgroundMessageHandler(RemoteMessage data) async {
    try {
      print("[Notification] Background message received: ${data.data}");
      final prefs = await SharedPreferences.getInstance();
      Constant.session = SessionManager(prefs: prefs);

      // Create a local instance if the static one isn't available
      final localNotificationInstance = LocalAwesomeNotification();

      if (Platform.isAndroid) {
        if (data.data["image"] == "" || data.data["image"] == null) {
          print(
              "[Notification] Creating default background notification (no image).");
          localNotificationInstance.createNotification(
              isLocked: false, data: data);
        } else {
          print("[Notification] Creating image background notification.");
          localNotificationInstance.createImageNotification(
              isLocked: false, data: data);
        }
      }
    } catch (e) {
      print("ISSUE in onBackgroundMessageHandler: ${e.toString()}");
    }
  }

  static foregroundNotificationHandler() async {
    try {
      print("[Notification] Foreground handler setup started.");
      onMessageOpen =
          FirebaseMessaging.onMessage.listen((RemoteMessage message) {
        print("[Notification] Foreground message received: ${message.data}");

        if (Platform.isAndroid) {
          if (message.data["image"] == "" || message.data["image"] == null) {
            print("[Notification] Creating default notification (no image).");
            localNotification?.createNotification(
                isLocked: false, data: message);
          } else {
            print("[Notification] Creating image notification.");
      ...
      print("ISSUE in foregroundNotificationHandler: ${e.toString()}");
    }
  }
...
// Declare a static counter
  ...
  static registerListeners(context) async {
    try {
      ...
      print('on background message handler started');
      FirebaseMessaging.onBackgroundMessage(onBackgroundMessageHandler);
      print('on background message handler ended');
